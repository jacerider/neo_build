...
- **Committed `neo.json` / dev lock** — `neo.json` + `neo.tsconfig.json` are generated (git-ignored); `_neo.lock` means dev mode is on. Use `neo-dev-cleanup` before a production build/commit.
