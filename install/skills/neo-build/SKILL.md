...
**An info file's `neo:` block accepts four Tailwind sections** — `theme`, `components`, `utilities` and `variants`. Anything else is dropped. `base:` is the **only** dropped key that says so: it is reported at prepare with a notice naming the extension and the key, and the build still succeeds. **Every other unrecognised key is ignored without comment**, so a typo there fails silently and the styling simply never appears.

⚠ **There is no base layer, from an info file or a subscriber.** `addTailwindBase()`, `getTailwindBase()` and `clearTailwindBase()` are gone from `NeoBuildCollection`, and the stylesheet has nowhere to route base-level data. Put **custom properties in the theme** (`neo: theme:`, or `addTailwindThemeItem()` from a subscriber) and **rules in components** (`neo: components:`, or `addTailwindComponents()`).
