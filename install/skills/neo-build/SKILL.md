...
**Base-layer Tailwind data is no longer accepted.** `addTailwindBase()`, `getTailwindBase()` and `clearTailwindBase()` are gone from `NeoBuildCollection`, and the stylesheet has no base layer to route to. `@layer base` reached none of the generated stylesheets on any site: `neo_font`, the only caller anywhere, had always passed an empty array. A subscriber that needs base-level declarations puts custom properties in the theme (`addTailwindThemeItem()`) and rules in components (`addTailwindComponents()`). This removed one of the eight collection signatures the preparer work published for sibling subscribers — announced rather than silent, and `NeoBuildCollectionContractTest` now pins seven. **The info-file key went the same way, one release later.** A `neo: base:` block in an extension's info file is now reported at prepare — a notice naming the extension and the key — and dropped; the build still succeeds, because the declaration reached nothing whether or not it was honoured. The same two destinations apply: custom properties into `neo: theme:`, rules into `neo: components:`. The four accepted sections are `theme`, `components`, `utilities` and `variants`, and prepare dispatches them by name rather than through `method_exists()`, so removing another collection method breaks a call site instead of going quiet. Unknown `neo:` keys are still ignored without comment. `base` is also gone from `neo.json` and from `tools/neo-tailwind-plugin.ts`, along with the plugin's unreachable `isMain` option.
