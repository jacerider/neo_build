...
class NeoBuildCollection {
  ...
  /**
   * The PHPStan configuration data.
   *
   * @var array
   */
  private array $stan = [
  ...
  /**
   * Adds a PHPStan path for an extension.
   *
   * @param \Drupal\neo_build\NeoExtension $extension
   *   The extension to add the PHPStan path for.
   *
   * @return $this
   *   The current instance for method chaining.
   */
  public function addStanPath(NeoExtension $extension): self {
    $this->stan['parameters']['paths'][$extension->getName()] = $this->getRelativeRoot() . $extension->getPath();
    return $this;
  }
  ...
  /**
   * Converts the PHPStan configuration to YAML format.
   *
   * @return string
   *   The PHPStan configuration as a YAML string.
   */
  public function toStanYaml(): string {
    $data = $this->stan;
    if (file_exists('modules/custom')) {
      $data['parameters']['paths']['customModules'] = $this->getRelativeRoot() . 'modules/custom';
    }
    if (file_exists($this->getRoot() . 'vendor/mglaman/phpstan-drupal/extension.neon')) {
      $data['includes'][] = 'vendor/mglaman/phpstan-drupal/extension.neon';
    }
    $data['parameters']['paths'] = array_values($data['parameters']['paths']);
    return "# This file is auto-generated by Neo\n" . Yaml::encode($data);
  }
